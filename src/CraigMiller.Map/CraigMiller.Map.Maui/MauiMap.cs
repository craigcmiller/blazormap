...
public class MauiMap
{
    ...
    /// </summary>
    public Action<MauiMap, MapEngine>? Setup { get; set; }

    public MapEngine Engine => _engine;

    internal void OnPaintSurface(SKImageInfo info, SKCanvas canvas)
    {
    ...
    internal void OnHandlerChanged(IViewHandler? handler, IDispatcher dispatcher, Action invalidateSurface)
    {
        if (handler != null)
        {
            if (Setup is null)
    ...
    internal void OnTouch(object? sender, SKTouchEventArgs e)
    {
        float density = (float)DeviceDisplay.Current.MainDisplayInfo.Density;
